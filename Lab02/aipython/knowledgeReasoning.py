...
from aipython.logicRelation import Var, Atom, Clause, KB, unify, apply
from aipython.knowledgeGraph import TripleStore, sts
import random
...
class KBT(KB):
    ...
    def eval_triple(self, ans, selected, remaining, indent):
        ...
        for tr in random.sample(retrieved,len(retrieved)):
            ...
            if sub is not False:
                yield from aipython.self.prove(apply(ans,sub), apply(remaining,sub), indent+"    ")

# simple test case:
kbt = KBT(sts)  # sts is simple triplestore from aipython.knowledgeGraph.py
