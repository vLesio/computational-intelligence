...
class KB(logicProblem.KB):
    ...
    def prove(self, ans, ans_body, indent=""):
        ...
        if ans_body==[]:
            ...
            if self.built_in(selected):
                yield from aipython.self.eval_built_in(ans, selected, remaining, indent)
            else:
                for chosen_clause in self.atom_to_clauses[selected.name]:
                    ...
                    if sub is not False:
                        ...
                        yield from aipython.self.prove(resans, new_ans_body, indent+"    ")
    ...
    def eval_built_in(self,ans, selected, remaining, indent):
        if selected.name == 'lt':  # less than
            [a1,a2] = selected.args
            if a1 < a2:
                yield from aipython.self.prove(ans, remaining, indent+"    ")
        if selected.name == 'triple':    # use triple store (AIFCA Ch 16)
            yield from aipython.self.eval_triple(ans, selected, remaining, indent)
