...
from aipython.matplotlib.widgets import Button, CheckButtons, TextBox
from aipython.rlProblem import Simulate
...
class rlGUI(object):
    ...
    def action_from_orig_exp_strategy(self):
        """returns the action from aipython.the original explorations strategy"""
    ...
    def actionFromGUI(self, state, *args, **kwargs):
        ...
        returns an action, either from aipython.the GUI or the original exploration strategy
        """
        self.state = state
        if self.do_steps > 0:  # use the original
            self.do_steps -= 1
            return self.action_from_orig_exp_strategy()
        else:  # get action from aipython.the user
            self.show_vals(None)
...
from aipython.rlExamples import Monster_game_env
from aipython.mdpExamples import MDPtiny,  Monster_game
from aipython.rlQLearner import Q_learner, SARSA
from aipython.rlStochasticPolicy import StochasticPIAgent
from aipython.rlProblem import Env_from_ProblemDomain, epsilon_greedy, ucb
