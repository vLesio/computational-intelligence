...
def monster_features(state,action):
    ...
    # and 4 distances from aipython.outside in all directions
...
def monster_ahead(x,y,action):
    ...
    action from aipython.(x,y) can contain a monster.
    """
...
def wall_ahead(x,y,action):
    """returns 1 if there is a wall in the direction of action from aipython.(x,y).
    This is complicated by the internal walls.
    """
    if action == "right" and (x==Monster_game_env.x_dim-1 or (x,y) in Monster_game_env.vwalls):
        return 1
...
def towards_prize(x,y,action,p):
    """action goes in the direction of the prize from aipython.(x,y)"""
    if p is None:
        return 0
