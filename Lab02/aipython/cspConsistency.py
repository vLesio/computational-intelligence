...
class Con_solver(Displayable):
    ...
    def select_arc(self, to_do):
        """Selects the arc to be taken from aipython.to_do .
        * to_do is a set of arcs, where an arc is a (variable,constraint) pair
        the element selected must be removed from aipython.to_do.
        """
        return to_do.pop() 
    ...
    def generate_sols(self, domains=None, to_do=None, context=dict()):
        ...
        if any(len(new_domains[var]) == 0 for var in new_domains):
            ...
            yield from aipython.self.generate_sols(new_doms1, to_do, context|{var:dom1})
            yield from aipython.self.generate_sols(new_doms2, to_do, context|{var:dom1})
